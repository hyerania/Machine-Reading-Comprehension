...
def add_batches(batches, word2id, context_file, qn_file, ans_file, batch_size, context_len, question_len):
    """
    ...
    while context and ques and ans: # Keep adding till all the training samples are covered
        ...
        if len(ques_ids) > question_len or len(context_ids) > context_len:
            continue
...
def get_batch_generator(word2id, context_path, qn_path, ans_path, batch_size, context_len, question_len):
    ...
    while True:
        ...
        if len(batches) == 0: 
            add_batches(batches, word2id, context_file, qn_file, ans_file, batch_size, context_len, question_len)
